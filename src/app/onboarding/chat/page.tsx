...
const ALL_STEPS: OnboardingStep[] = [
  {
    id: 'degreeLevel',
    question:
      "Hi! I'm Maya, your AI study abroad counselor. Let's get to know you. What degree are you planning to pursue?",
    field: 'degreeLevel',
    type: 'options',
  ...
  },
  {
    id: 'currentEducation',
    question:
      "What's your current education? For example, 'B.Tech in CS from IIT Delhi'",
  ...
  },
  {
    id: 'hasGivenTests',
    question:
      'Have you taken any standardized tests like GRE, IELTS, or TOEFL?',
    field: 'hasGivenTests',
    type: 'yes_no',
  },
  {
    id: 'testScores',
    question:
      "Add your test scores below. Click 'Add Exam' for each test you've taken.",
    field: 'testScores',
    type: 'test_scores',
  },
];
...
export default function OnboardingChat() {
  ...
  const getVisibleSteps = useCallback(() => {
    return ALL_STEPS.filter((step) => {
      if (
        step.id === 'intakeSeason' &&
        onboardingData.intakeYear === 'Not Decided'
      )
        return false;
      if (step.id === 'testScores' && onboardingData.hasGivenTests === false)
        return false;
      return true;
    });
  ...
  const handleOptionClick = (option: string) => {
    ...
    const value = currentStepData.optionMap
      ? currentStepData.optionMap[option]
      : option;
  ...
  const handleOtherSubmit = (e: React.FormEvent) => {
    ...
    if (currentStepData.id === 'degreeLevel') {
      setOnboardingData((prev) => ({
        ...
        degreeLevelCustom: otherInput.trim(),
      }));
    } else {
      setOnboardingData((prev) => ({
        ...prev,
        [currentStepData.field]: otherInput.trim(),
      }));
    }
    setShowOptions(false);
    setShowOtherInput(false);
  ...
  const handleTestScoresSubmit = () => {
    const valid = onboardingData.testScores.filter(
      (ts) => ts.examType && ts.overallScore
    );
  ...
  const completeOnboarding = async () => {
    ...
    speakMessage(
      "Excellent! I've got everything I need. Let me set up your personalized experience..."
    );

    try {
      const degreeRaw = onboardingData.degreeLevel;
      const isStandardDegree =
        degreeRaw && ['UG', 'PG', 'MBA', 'PHD'].includes(degreeRaw);
      ...
      const payload: Record<string, any> = {
        ...
        intakeYear:
          onboardingData.intakeYear === 'Not Decided'
            ? null
            : onboardingData.intakeYear,
        budgetRange: onboardingData.budgetRange,
        hasGivenTests: onboardingData.hasGivenTests,
      };

      if (
        onboardingData.hasGivenTests &&
        onboardingData.testScores.length > 0
      ) {
  ...
  const hasValidScores = onboardingData.testScores.some(
    (ts) => ts.examType && ts.overallScore
  );
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#0a0a0a] relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center w-full max-w-4xl px-4 pb-48">
        ...
        <AnimatePresence mode="wait">
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {currentStepData.options.map((option, index) => {
                        const isSecondary =
                          option === 'Not Decided' || option === 'Other';
                        return (
                          <motion.button
                  ...
                  >
                    ...
                    <form onSubmit={handleOtherSubmit} className="flex gap-3">
                      <input
                        ...
                        placeholder={
                          currentStepData.otherPlaceholder || 'Type here...'
                        }
                        autoFocus
                    ...
                    <button
                      ...
                      className="mt-3 text-sm text-gray-500 hover:text-gray-300 transition-colors mx-auto block"
                    >
          ...
          {showOptions &&
            !isProcessing &&
            currentStepData.type === 'yes_no' && (
              <motion.div
                key={`yesno-${currentStepIndex}`}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -20 }}
          ...
          {showOptions &&
            !isProcessing &&
            currentStepData.type === 'test_scores' && (
              <motion.div
                key={`scores-${currentStepIndex}`}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -20 }}
