...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: 'Total Universities', value: stats?.counts?.universities || 0, icon: GraduationCap },
    { label: 'Total Programs', value: stats?.counts?.programs || 0, icon: Database },
    { label: 'Total Students', value: stats?.counts?.students || 0, icon: Users },
    { label: 'Total Applications', value: stats?.counts?.applications || 0, icon: ClipboardList },
  ]
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {isLoading ? (
            Array(4).fill(0).map((_, i) => (
              <div key={i} className="bg-[#0f0f0f] border border-gray-800 p-6 rounded-2xl animate-pulse">
            ...
            statCards.map((stat) => (
              <div key={stat.label} className="bg-[#0f0f0f] border border-gray-800 p-6 rounded-2xl hover:border-primary/30 transition-all group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
              ...
              {quickActions.map((action) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-white group-hover:text-primary transition-colors">{action.title}</h3>
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-white">Recent University Additions</h2>
              <Link href="/admin/universities" className="text-primary text-xs hover:underline flex items-center gap-1">
              ...
              <div className="divide-y divide-gray-800">
                {isLoading ? (
                  ...
                  stats?.recentUniversities?.map((u: any) => (
                    <div key={u.id} className="p-4 flex items-center justify-between hover:bg-[#151515] transition-colors group">
                      <div className="flex items-center gap-4">
                        <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold text-sm shadow-lg", u.logoColor || 'bg-gray-700')}>
                          {u.logo}
                        </div>
                        <div>
