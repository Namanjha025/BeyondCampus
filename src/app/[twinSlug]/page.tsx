...
export default function PublishedTwin() {
  ...
  const initializeTwinChat = async () => {
    try {
      ...
      const fullMessage = res.ai_text || `Hello! I'm ${twinData.twinName || twinSlug.replace('-', ' ')}. How can I help you today?`
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const res = await sendTwinChat(twinData.id, visitorMessage.content, threadId)
      const fullMessage = res.ai_text || "I apologize, but I'm having trouble responding right now. Please try again."
      setMessages(prev => prev.map(msg => 
      ...
      setMessages(prev => prev.map(msg => 
        msg.id === twinMessageId 
          ? { ...msg, content: "I apologize, but I'm having trouble responding right now. Please try again." }
          : msg
      ))
      setIsTyping(false)
      setIsLoading(false)
          ...
          <div className="space-y-4">
            {messages.map((message) => (
              ...
              >
                ...
                <div className={`max-w-[80%] ${message.role === "visitor" ? "flex flex-col items-end" : ""}`}>
                  {message.role === "twin" && (
              ...
              <div className="flex gap-3">
                ...
                <div className="bg-[hsl(0_0%_8%)] border border-[hsl(0_0%_18%)] rounded-lg px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">AI Twin is typing...</span>
                  </div>
                </div>
              </div>
            )}
